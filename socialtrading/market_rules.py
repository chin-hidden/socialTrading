...
class PriceService:
    BASE_URL = "http://priceservice.vndirect.com.vn/priceservice"

    def parse_stock_info(self, line):
        arr = line.split("|")
        return Stock(
            symbol=arr[3],
            company_name=arr[4],
    ...
    def get_stock_info(self, stock_symbol):
        res = requests.get(self.BASE_URL + "/secinfo/snapshot/q=codes:" + stock_symbol)
        return self.parse_stock_info(res.json()[0])
...
def get_stock_from_symbol(symbol: str):
    ...
    stock = PriceService().get_stock_info(symbol)
    stocks.append(stock)
    return stock
